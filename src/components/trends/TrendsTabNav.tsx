...
export function TrendsTabNav({ active, onChange, savedCount = 0 }: TrendsTabNavProps) {
  return (
    <nav
      className="flex gap-1 rounded-xl border border-zinc-800/60 bg-zinc-950/50 p-1"
    ...
    >
      {TABS.map(({ id, label, icon: Icon }) => (
        <button
          ...
          className={cn(
            'relative flex min-h-10 flex-1 items-center justify-center gap-1.5 rounded-lg px-3 py-2 text-xs font-medium transition-smooth sm:text-sm',
            active === id
