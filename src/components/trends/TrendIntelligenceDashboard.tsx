...
function MetricPill({
  ...
  return (
    <div
      className={cn(
        'min-w-[7rem] shrink-0 rounded-2xl border px-3.5 py-3 backdrop-blur-md',
        accent
...
function SectionCarousel({
  ...
  return (
    <section className="ti-dashboard-section min-w-0">
      ...
      <div className="ti-dashboard-scroll flex gap-2.5 overflow-x-auto pb-1 scroll-smooth-mobile snap-x snap-mandatory [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {items.map((item) => {
          ...
          return (
            <button
              ...
              className={cn(
                'ti-dashboard-chip snap-start shrink-0 text-left',
                'w-[min(82vw,16rem)] rounded-2xl border border-zinc-800/55 bg-zinc-950/65 p-3.5',
...
function TrendIntelligenceDashboardInner({
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-zinc-500">
            {trends.length} live signals · {snapshot.topPlatform} leading
          </p>
        </div>
      ...
      <div className="ti-dashboard-metrics flex gap-2 overflow-x-auto pb-0.5 scroll-smooth-mobile snap-x snap-mandatory [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        <MetricPill label="Avg Opportunity" value={snapshot.avgOpportunity} accent />
        <MetricPill label="Exploding" value={snapshot.explodingCount} />
        <MetricPill label="Rising" value={snapshot.risingCount} />
        <MetricPill label="Top Platform" value={snapshot.topPlatform} />
      </div>
