  ...
  if (value === true) {
    return (
      <span className="inline-flex items-center justify-center text-emerald-400">
        <CheckIcon className="size-4" aria-label="Enthalten" />
      </span>
    )
  }
  if (value === false) {
        ...
        <table className="w-full min-w-[640px] border-collapse text-left text-[11px]">
          <thead>
            <tr className="border-b border-zinc-800/60 bg-zinc-900/40">
              <th
                scope="col"
                className="sticky left-0 z-10 bg-zinc-900/95 px-3 py-2.5 font-semibold text-zinc-500 backdrop-blur-sm sm:px-4 sm:py-3"
              >
                Feature
              </th>
              {COMPARISON_PLAN_COLUMNS.map((id) => {
                ...
                return (
                  <th
                    ...
                    scope="col"
                    className={cn(
                      'px-2 py-2.5 text-center font-semibold sm:px-3 sm:py-3',
              ...
              return (
                <Fragment key={row.label}>
                  {showCategory && (
                    <tr className="bg-zinc-900/25">
                      <td
                        ...
                        className="px-3 py-1.5 text-[9px] font-bold uppercase tracking-[0.14em] text-violet-400/90 sm:px-4 sm:py-2"
                      >
                        {row.category}
                      </td>
                    </tr>
                  ...
                  <tr className="border-t border-zinc-800/35 transition-colors hover:bg-violet-500/[0.03]">
                    <td className="sticky left-0 z-10 bg-zinc-950/95 px-3 py-2 font-medium text-zinc-300 backdrop-blur-sm sm:px-4 sm:py-2.5">
                      {row.label}
                    </td>
                    {COMPARISON_PLAN_COLUMNS.map((id) => (
                      <td
