...
DECLARE
  ...
  email text;
BEGIN
  IF EXISTS (SELECT 1 FROM public.ai_generations WHERE email LIKE '%@nextrends.app' LIMIT 1) THEN
    RETURN;
  END IF;

  FOR i IN 1..120 LOOP
    ts := now() - (random() * interval '30 days');
    tool := demo_tools[1 + floor(random() * array_length(demo_tools, 1))::int];
    niche := demo_niches[1 + floor(random() * array_length(demo_niches, 1))::int];
    plat := demo_platforms[1 + floor(random() * array_length(demo_platforms, 1))::int];
    email := demo_emails[1 + floor(random() * array_length(demo_emails, 1))::int];
    ...
    INSERT INTO public.ai_generations (
      ...
      niche,
      plat,
      'Demo prompt for ' || niche || ' on ' || plat,
      1 + floor(random() * 2)::int,
      ts
    );
  END LOOP;

  -- Backfill analytics_daily from seeded rows
  INSERT INTO public.analytics_daily (day, total_generations, credits_consumed, active_users)
  SELECT
  ...
  WHERE email LIKE '%@nextrends.app'
  GROUP BY 1
  ON CONFLICT (day) DO UPDATE SET
  ...
  SELECT
    ...
    max(created_at)
  FROM public.ai_generations
  WHERE platform <> '' AND email LIKE '%@nextrends.app'
  GROUP BY platform
  ON CONFLICT (platform) DO UPDATE SET
